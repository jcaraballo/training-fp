...
trait MonadLaws extends FunctorLaws {
  def monadSatisfiesLaws[F[_], A, B, C](implicit monad: Monad[F],
                                             arbA: Arbitrary[A],
                                             arbFA: Arbitrary[F[A]],
    ...
    functorSatisfiesLaws[F, A, B, C]
    monadSatisfiesLeftIdentityLaw[F, A, B]
    monadSatisfiesRightIdentityLaw[F, A]
    monadSatisfiesAssociativityLaw[F, A, B, C]
  }

  def monadSatisfiesLawsWithKleisliComposition[F[_], A, B, C, D](implicit monad: Monad[F],
                                             arbA: Arbitrary[A],
                                             arbFA: Arbitrary[F[A]],
                                             arbAtoB: Arbitrary[A => B],
                                             arbBtoC: Arbitrary[B => C],
  ...
  private def monadSatisfiesLeftIdentityLaw[F[_], A, B](implicit monad: Monad[F], arbA: Arbitrary[A], arbAtoFB: Arbitrary[A => F[B]]): Unit =
    forAll { (a: A, op: A => F[B]) =>
      monad.flatMap(monad.pure(a))(op) shouldBe op(a)
    }

  private def monadSatisfiesRightIdentityLaw[F[_], A](implicit monad: Monad[F], arbFA: Arbitrary[F[A]]): Unit =
    forAll { fa: F[A] =>
      monad.flatMap(fa)(monad.pure) shouldBe fa
    }

  private def monadSatisfiesAssociativityLaw[F[_], A, B, C](
  ...
  def monadSatisfiesLeftIdentityLawWithKleisliComposition[F[_], A, B](implicit monad: Monad[F],
                                                                      arbA: Arbitrary[A],
                                                                      arbAtoFB: Arbitrary[A => F[B]]
                                                                     ): Unit =
    forAll { (a: A, op: A => F[B]) =>
  ...
  def monadSatisfiesRightIdentityLawWithKleisliComposition[F[_], A, B](implicit monad: Monad[F],
                                                                       arbA: Arbitrary[A],
                                                                       arbAtoFB: Arbitrary[A => F[B]]
                                                                      ): Unit =
    forAll { (a: A, op: A => F[B]) =>
